...
- **Anything not listed above belongs to the document, not to you.** The rules in this section are the standard's and they don't move. Everything else — the Version History table's own font and borders, body text size, spacing, column widths, the table style — is the document's own house style: read it off the document and match it. Never let an edit fall back to the application's defaults. That is what produces a row or a paragraph rendering larger, or in a different colour, than everything around it: in these documents font, size and colour usually sit on the runs rather than on the style, so anything written without them inherits Word's defaults instead of the document's look. The bullet-vs-number rule above is this same principle applied to a single property.
- **When a document's house style conflicts with a rule above, ask — don't silently normalise.** A base whose titles are all 14pt isn't following this standard, and bringing it into line is a content edit like any other, with the version bump that implies. Offer both: standardise it, or keep the house style knowing `check_master_list.py` will report it. Ask **only** when there is a real conflict — never on an edit where the standard is silent, because then there is nothing to decide.
